...
const COPY = {
  ja: {
    ...
    description:
      "店名と希望日時を送るだけ。空席確認は無料で、予約できる場合のみ先払いで確定します。",
    formLabel: "予約リクエスト",
    formTitle: "予約したいお店を入力",
    formDescription:
      "店名・希望日時・メールアドレスだけで依頼できます。住所やURLがあると、店舗をより正確に特定できます。",
    confirmationNote:
      "この段階では予約者名は不要です。予約可能と確認できた場合、メールで予約名義と先払い方法をご案内します。",
    required: "必須",
    optional: "任意",
    ...
    email: "メールアドレス",
    emailHelp:
      "受付内容と確認結果をお送りします。予約可能な場合は、予約名義の確認とお支払い方法をご案内します。",
    consent: "予約条件、キャンセル・ノーショー規定、個人情報の利用に同意します。",
    submit: "無料で空席確認を依頼する",
    submitting: "送信中…",
    successTitle: "予約リクエストを受け付けました",
    successSent:
      "受付内容をメールでお送りしました。現在、店舗へ予約可能か確認しています。",
    successNotSent:
      "リクエストは受け付けました。確認メールを送信できなかったため、受付番号を保存してください。",
    requestCode: "受付番号",
    another: "別の予約を依頼する",
    ...
    paymentText:
      "空席がなければ請求はありません。予約できる場合のみ、内容と金額をご確認いただいたうえで先払いします。",
    paymentItems: [
      "代行手数料：目安 ¥700〜",
      "店舗予約金：店舗が求める実費",
      "決済：PayPal・対応カード",
    ...
    noShow:
      "予約確定後の代行手数料は返金されません。店舗予約金は、店舗のキャンセル・ノーショー規定に従います。",
    exploreLabel: "お店を探す",
    exploreTitle: "まだお店が決まっていませんか？",
    exploreDescription:
      "カテゴリーを押すと、日本語の検索語でNaver Mapが開きます。店名と住所をコピーして、予約フォームに入力してください。",
    exploreNote: "Naver Mapの画面言語は、Naverまたはブラウザの設定に従います。",
    footer: "Booking Daijoubu · Seoul booking concierge",
    footerNotice:
      "予約成立や店舗のサービス品質を保証するものではありません。医療予約には対応していません。",
  },
  en: {
    ...
    description:
      "Send the venue and preferred time. Availability checks are free; payment is requested only if the venue can accept your booking.",
    formLabel: "Booking request",
    formTitle: "Enter the place you want to book",
    formDescription:
      "Start with the venue, preferred time, and your email. An address or link helps us identify the correct location.",
    confirmationNote:
      "You do not need to provide a booking name yet. If the venue is available, we’ll email you to request the name and send prepayment details.",
    required: "Required",
    optional: "Optional",
    ...
    email: "Email",
    emailHelp:
      "We’ll send your request summary and availability result. If available, we’ll also request the booking name and send payment instructions.",
    ...
    successSent:
      "We emailed you a copy of the request. We are now checking availability with the venue.",
    successNotSent:
      "Your request was saved, but the confirmation email could not be sent. Please keep the request code.",
    ...
    paymentText:
      "You pay nothing if the venue is unavailable. If it can accept the booking, you review the details and prepay before we finalize it.",
    paymentItems: [
      "Service fee: from approximately JPY 700",
      "Venue deposit: the amount required by the venue",
      "Payment: PayPal or supported cards",
    ],
    noShow:
      "The service fee is non-refundable after confirmation. Venue deposits follow the venue’s cancellation and no-show policy.",
    exploreLabel: "Find a place",
    exploreTitle: "Still deciding where to go?",
    exploreDescription:
      "Select a category to open an English search on Naver Map. Copy the venue name and address, then paste them into the form.",
    exploreNote: "The Naver Map interface language follows your Naver or browser settings.",
    footer: "Booking Daijoubu · Seoul booking concierge",
    footerNotice:
      "We cannot guarantee availability or the venue’s service quality. Medical bookings are not supported.",
  },
} as const;
...
export default function BookingHome() {
  ...
  const validation = useMemo<Record<ValidationKey, string>>(() => {
    const preferredAt = preferredDate
      ? new Date(`${preferredDate}T${form.preferredTime || "00:00"}:00+09:00`)
      : null;
    const dateInvalid =
      !preferredDate ||
      !preferredAt ||
    ...
    return {
      ...
      partySize:
        Number.isInteger(partySize) && partySize >= 1 && partySize <= 20
          ? ""
          : copy.requiredError,
      customerEmail: EMAIL_PATTERN.test(form.customerEmail.trim())
        ? ""
        : copy.emailError,
  ...
  const yearSelect = (
    <select
      ...
      onChange={(event) => update("preferredYear", event.target.value)}
    >
      {YEARS.map((year) => (
        <option key={year} value={year}>
          {language === "ja" ? `${year}年` : year}
        </option>
      ))}
    </select>
  );
  ...
  const monthSelect = (
    <select
      ...
      onChange={(event) => update("preferredMonth", event.target.value)}
    >
    ...
    <main className="booking-page">
      <header className="booking-header">
        ...
        <nav className="booking-nav">
          <button
            className="booking-nav-action booking-nav-primary"
            type="button"
            onClick={() => scrollTo("booking-request")}
          >
            {copy.navRequest}
          </button>
          <button
            className="booking-nav-action"
            type="button"
            onClick={() => scrollTo("booking-explore")}
          >
            {copy.navExplore}
          </button>
      ...
      <section className="booking-concern-strip" aria-label="Booking concerns">
        <div className="booking-concern-track">
          {[...CONCERNS[language], ...CONCERNS[language]].map(
            ([icon, text], index) => (
              <span
                key={`${text}-${index}`}
      ...
      <div className="booking-platform-shell">
        <section className="booking-platform-intro booking-compact-intro">
          <h1>{copy.title}</h1>
          ...
          <div className="booking-form-card booking-platform-card">
            ...
            {requestCode ? (
              <div className="booking-success">
                ...
                <p>{emailSent ? copy.successSent : copy.successNotSent}</p>
                <div>
                  <small>{copy.requestCode}</small>
              ...
              <form noValidate onSubmit={handleSubmit}>
                <label className="booking-field booking-field-wide">
                  <span>
                    {copy.category}
                    <b className="booking-required">{copy.required}</b>
                  </span>
                ...
                <label
                  className={`booking-field booking-field-wide ${
                    hasError("placeName") ? "has-error" : ""
                  }`}
                  data-error={hasError("placeName") || undefined}
                >
                  <span>
                    {copy.placeName}
                    <b className="booking-required">{copy.required}</b>
                  ...
                  <input
                    aria-invalid={hasError("placeName")}
                    value={form.placeName}
                    onChange={(event) => update("placeName", event.target.value)}
                    placeholder={copy.placeNamePlaceholder}
                    maxLength={120}
                  />
                  {hasError("placeName") && (
                    <small className="booking-validation-error">
                      {validation.placeName}
                    </small>
                  )}
                </label>
                ...
                <label className="booking-field booking-field-wide booking-optional-field">
                  <span>
                    {copy.placeAddress}
                    <em className="booking-optional">{copy.optional}</em>
                  </span>
                  <input
                    value={form.placeAddress}
                    onChange={(event) => update("placeAddress", event.target.value)}
                    placeholder={copy.placeAddressPlaceholder}
                    maxLength={300}
                  />
                  <small className="booking-field-help">
                    {copy.placeAddressHelp}
                  </small>
                </label>
                ...
                <fieldset
                  className={`booking-fieldset booking-field-wide ${
                    hasError("preferredDate") || hasError("preferredTime")
                      ? "has-error"
                      : ""
                  }`}
                  data-error={
                ...
                >
                  ...
                  <div className={`booking-date-grid ${language}`}>
                    {language === "ja"
                  ...
                  {(hasError("preferredDate") ||
                    hasError("preferredTime")) && (
                    <small className="booking-validation-error">
                      {validation.preferredDate || validation.preferredTime}
                    </small>
                  )}
                </fieldset>
                ...
                <label
                  className={`booking-field ${
                    hasError("partySize") ? "has-error" : ""
                  }`}
                  data-error={hasError("partySize") || undefined}
                >
                  <span>
                    {copy.partySize}
                    <b className="booking-required">{copy.required}</b>
                  ...
                  <input
                    aria-invalid={hasError("partySize")}
                    type="number"
                    min="1"
                    max="20"
                ...
                <label className="booking-field booking-field-wide booking-optional-field">
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      update("requestDetails", event.target.value)
                    }
                ...
                <label
                  className={`booking-field booking-field-wide ${
                    hasError("customerEmail") ? "has-error" : ""
                  }`}
                  data-error={hasError("customerEmail") || undefined}
                >
                  <span>
                    {copy.email}
                    <b className="booking-required">{copy.required}</b>
                  ...
                  <input
                    ...
                    maxLength={180}
                  />
                  <small className="booking-field-help">{copy.emailHelp}</small>
                ...
                <label
                  className={`booking-consent booking-field-wide ${
                    hasError("consent") ? "has-error" : ""
                  }`}
                  data-error={hasError("consent") || undefined}
                >
                  <input
                ...
                {hasError("consent") && (
                  <small className="booking-validation-error booking-field-wide">
                    {validation.consent}
                  </small>
                )}
                {error && (
                ...
                <button
                  className="booking-submit booking-field-wide"
                  disabled={submitting}
                  type="submit"
                >
        ...
        <section
          id="booking-explore"
          className="booking-discovery booking-platform-card"
        >
          ...
          <div className="booking-marquee" aria-label={copy.exploreLabel}>
            {discoveryRows.map((row, rowIndex) => (
              <div
                className={`booking-marquee-lane ${
                  rowIndex === 1 ? "reverse" : ""
                }`}
                key={rowIndex}
              >
